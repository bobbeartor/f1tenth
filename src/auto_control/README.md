...
3. 네 개 수평 구간에서 먼저 좌우 차선 경계 쌍을 고른다. 허용 차로 폭을
   좁게 제한해 한 차선의 두 조각이나 중앙 표시와 경계가 좌우 한 쌍으로
   선택되지 않게 한다.
4. 현재 프레임에서 찾은 좌우 쌍으로 중심 곡선을 먼저 만든 다음, 한쪽
   경계만 보이는 구간의 좌/우를 분류한다. 최근 측정 폭(아직 측정하지
   못했으면 설정 폭)의 절반만큼 이동해 중앙선을 복원한다.
5. 가까운 중심은 횡오차로 사용하고, 측정 구간보다 먼 `lookahead_y_ratio`
   위치의 중심은 진행방향·선행 오차로 사용한다. 원근 수렴 때문에
   `far-near`를 방향 오차로 직접 쓰지 않는다.
...
차로 중심이다. `LOOKAHEAD` 원은 실제 검출점이 아니라 네 측정 중심을
외삽한 먼 조향 목표점이다. 상단의 `pairs` 값과 모든 scan 높이에서 노란
점이 실제 차로 중앙에 놓이는지 확인한다.
...
- `scan_y_ratios`: 전체 마스크에서 차선 위치를 측정할 네 개 높이다.
  차선 두 경계가 안정적으로 보이는 범위를 지정한다.
- `lookahead_y_ratio`: 측정 중심 곡선으로 예측하는 먼 조향 목표 높이다.
  값이 작을수록 더 먼 지점을 본다. 검출 밴드는 이동하지 않는다.
- `lane_width_far_ratio`, `lane_width_near_ratio`: 가장 먼/가까운 측정
  높이에서 보이는 좌우 경계 간격을 영상 폭으로 나눈 값이다. 한쪽 경계만
  검출될 때 중앙선 복원에도 사용된다.
- `pair_minimum_width_scale`, `pair_maximum_width_scale`: 설정 차로 폭 대비
  좌우 한 쌍으로 인정할 최소·최대 간격이다.
- `reset_after_missed_frames`: 현재 영상에서 좌우 차선 쌍을 연속으로 찾지
  못했을 때 저장된 중심/차로 폭을 초기화할 프레임 수다.
