...
`/lane_mask`에서 곡선 차로 중앙 경로를 만들고 차량을 기본 100 Hz로 조향하는
ROS 2 패키지다. VESC 명령은 기본적으로 비활성화되어 있다.

## 처리 방식

1. 입력 마스크를 고정 좌표계 `160x100`으로 정규화한다.
2. `y=60..83`(양 끝 포함) 밖의 픽셀을 제거한다. 특히 바퀴가 보이는 화면
   아래 1/6(`y=84..99`)은 차선 후보로 사용하지 않는다.
...
`config/centerline.yaml`에서 조정할 수 있다. 차로 폭은 카메라 위치 변화에도
같은 의미를 유지하도록 화면 y 비율과 영상 폭 대비 차로 폭 비율 3점으로
설정하며, 검출기는 세 점을 통과하는 2차 원근 곡선을 사용한다.
